...
namespace RecipeTest
{
    public class RecipeTest
    {
        ...
        [Test]
        public void DeleteRecipeThatsNotDraftedOrArchivedFor30Days()
        {
            ...
            Assume.That(recipeid > 0, "no currently published recipes and no recipes archived for less than 31 days in DB, cant run test");
            TestContext.WriteLine("existing recipe archived less than 31 days or currently published with id = " + recipeid + " " + recipedesc);
            TestContext.WriteLine("ensure that app cannot delete " + recipeid);
            Exception ex = Assert.Throws<Exception>(() => Recipe.Delete(dt));
            TestContext.WriteLine(ex.Message);
        }
